...
- Captured site regression snapshots: 59 (56 recovered captures, one harness fixture, and two new live captures)
...
The original 56 FeedFlow captures are retained as ordinary application-independent regressions, alongside the portable harness fixture and two new live captures. `SiteRegressionTest` compares both Markdown and cleaned HTML for all 59 cases on every target.
...
- `statista-global-sales-of-clorox-by-segment`: verifies that Statista's anonymous chart preview is promoted to the full 754-pixel infographic while pricing, login, and recommendation chrome is excluded.
