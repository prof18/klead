...
2026-06-15 - COMMIT-029 - complete - Converted network extractor plumbing to a real coroutine contract with `parseHtmlAsync`, suspend extractors, suspend injected HTTP clients, and a configurable parse dispatcher defaulting to `Dispatchers.Default`; `parseHtml` remains as a blocking compatibility wrapper.
...
2026-06-15 - PHASE-06 - complete - Added a Mashable deal FeedFlow reader-dump regression and removed byline author flyouts, freelance-writer profile boxes, default author placeholder images, and inline newsletter prompts from reader Markdown.
2026-06-16 - PHASE-06 - complete - Added a GameSpot FeedFlow reader-dump regression and removed article right-rail/sidebar commerce widgets so "Where to Buy" loading placeholders and commission disclosures do not leak into reader Markdown.
2026-06-16 - PHASE-06/07/08/11 - complete - Added a GamingOnLinux FeedFlow reader-dump regression, normalized YouTube consent placeholders into safe cleaned-HTML embeds with Markdown video links, preserved only trusted YouTube embed iframes through sanitization, and removed article-likes/social-comment/rules footer chrome from reader Markdown.
2026-06-16 - PHASE-06 - complete - Added a Rolling Stone FeedFlow reader-dump regression and removed article header/dek/date chrome plus in-article trending story recirculation from reader Markdown.
